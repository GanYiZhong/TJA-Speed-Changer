#!/usr/bin/env python3
"""
TJA Speed Changer GUI - Enhanced Version with Better Drag & Drop
A graphical interface with improved drag and drop support using tkinterdnd2
"""
...
class TJAProcessor:
    """Handles TJA file processing logic with OGG conversion"""
    ...
    def adjust_tja_speed(self, tja_path, speed, progress_callback=None):
        """Adjust TJA file speed parameters with OGG format enforcement"""
        ...
        for line in lines:
            ...
            # Modify WAVE filename - ALWAYS convert to OGG
        ...
        # Return the new wave filename (always OGG now)
        if wave_filename:
    ...
    def find_audio_file(self, base_dir, wave_filename):
        """Find audio file with various extensions (mp3, wav, ogg, flac, m4a)"""
    ...
    def process_files(self, tja_path, speed, progress_callback=None, log_callback=None):
        """Main processing function with OGG conversion"""
        try:
            if log_callback:
...
class TJASpeedChangerGUI:
    """Enhanced GUI application with better drag and drop"""
    ...
    def setup_file_selection(self, parent, row):
        ...
        self.drag_drop_label.grid(row=1, column=0, columnspan=2, pady=(5, 0), sticky=(tk.W, tk.E))
    ...
    def setup_drag_drop(self):
        ...
        if HAS_TKINTERDND2:
            # Use tkinterdnd2 for better drag and drop
            self.root.drop_target_register(tkdnd.DND_FILES)
            self.root.dnd_bind('<<DropEnter>>', self.on_drop_enter)
            self.root.dnd_bind('<<DropLeave>>', self.on_drop_leave)
            self.root.dnd_bind('<<Drop>>', self.on_drop)
        else:
            # Fallback: clickable drop zone
            self.drag_drop_label.bind('<Button-1>', lambda e: self.browse_file())
            self.drag_drop_label.bind('<Enter>', self.on_label_enter)
            self.drag_drop_label.bind('<Leave>', self.on_label_leave)
    
    def on_drop_enter(self, event):
        """Handle drag enter event"""
        self.drag_drop_label.config(relief='sunken', foreground='blue')
        
    def on_drop_leave(self, event):
        """Handle drag leave event"""
        self.drag_drop_label.config(relief='groove', foreground='gray')
    ...
    def on_label_enter(self, event):
        """Handle label enter for clickable version"""
        self.drag_drop_label.config(foreground='blue')
        
    def on_label_leave(self, event):
        """Handle label leave for clickable version"""
        self.drag_drop_label.config(foreground='gray')
    ...
    def process_files(self):
        """Process TJA files with OGG conversion"""
        # Validate input
